...
public abstract class BaseAction<T extends BaseSessionAwareDTO> extends ActionSupport
	...
	static {
		...
		MenuStructure mnuhomeMenu = new MenuStructure("mnuhome");

		menuMap.add(mnuhomeMenu);

		MenuStructure mnuRicCertificatoriMenu = new MenuStructure("mnuRicCertificatori");

		menuMap.add(mnuRicCertificatoriMenu);

		MenuStructure mnuRicCertificatoriAdvMenu = new MenuStructure("mnuRicCertificatoriAdv");

		menuMap.add(mnuRicCertificatoriAdvMenu);

		MenuStructure mnuRicACEMenu = new MenuStructure("mnuRicACE");

		menuMap.add(mnuRicACEMenu);

		MenuStructure mnuStampaAceMenu = new MenuStructure("mnuStampaAce");

		menuMap.add(mnuStampaAceMenu);

		MenuStructure mnuDatiEnteFormativoMenu = new MenuStructure("mnuDatiEnteFormativo");

		MenuStructure mnuDatiRegistrazioneMenu = new MenuStructure("mnuDatiRegistrazione");

		mnuDatiEnteFormativoMenu.getSubmenus().add(mnuDatiRegistrazioneMenu);

		menuMap.add(mnuDatiEnteFormativoMenu);

		MenuStructure mnuGestioneCorsiPartecipantiMenu = new MenuStructure("mnuGestioneCorsiPartecipanti");

		MenuStructure mnuSchedaCorsiMenu = new MenuStructure("mnuSchedaCorsi");

		mnuGestioneCorsiPartecipantiMenu.getSubmenus().add(mnuSchedaCorsiMenu);

		menuMap.add(mnuGestioneCorsiPartecipantiMenu);

		MenuStructure mnuBackOfficeMenu = new MenuStructure("mnuBackOffice");

		MenuStructure mnuTransazioniMdpMenu = new MenuStructure("mnuTransazioniMdp");

		mnuBackOfficeMenu.getSubmenus().add(mnuTransazioniMdpMenu);
		MenuStructure mnuVerificaIndexMenu = new MenuStructure("mnuVerificaIndex");

		mnuBackOfficeMenu.getSubmenus().add(mnuVerificaIndexMenu);
	...
	private it.csi.sicee.siceebo.business.BackEndFacade backEnd = null;

	public void setSpringBackEnd(it.csi.sicee.siceebo.business.BackEndFacade be) {
		backEnd = be;
	}

	public it.csi.sicee.siceebo.business.BackEndFacade getSpringBackEnd() {
		return backEnd;
	}

	private it.csi.sicee.siceebo.business.SecurityHelper securityHelper = null;

	public void setSpringSecurityHelper(it.csi.sicee.siceebo.business.SecurityHelper sh) {
		securityHelper = sh;
	}

	public it.csi.sicee.siceebo.business.SecurityHelper getSpringSecurityHelper() {
