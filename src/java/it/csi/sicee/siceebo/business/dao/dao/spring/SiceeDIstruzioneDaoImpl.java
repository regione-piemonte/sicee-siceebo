...
import it.csi.sicee.siceebo.business.dao.dto.SiceeDIstruzionePk;
...
public class SiceeDIstruzioneDaoImpl extends AbstractDAO implements ParameterizedRowMapper<SiceeDIstruzione>, SiceeDIstruzioneDao
{
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_ISTRUZIONE, FK_TIPO_ISTRUZIONE, CODICE, DENOMINAZIONE, SIGLA_ORDINE, SIGLA_NON_ORDINE FROM " + getTableName() + " ORDER BY DENOMINAZIONE", this);
		}
		catch (Exception e) {
			throw new SiceeDIstruzioneDaoException("Query failed", e);
		}
