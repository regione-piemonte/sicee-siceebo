...
import it.csi.sicee.siceebo.business.dao.dao.SiceeTDocAggiuntivaDao;
import it.csi.sicee.siceebo.business.dao.dto.SiceeTDocAggiuntiva;
import it.csi.sicee.siceebo.business.dao.dto.SiceeTDocAggiuntivaPk;
...
public class SiceeTDocAggiuntivaDaoImpl extends AbstractDAO implements ParameterizedRowMapper<SiceeTDocAggiuntiva>, SiceeTDocAggiuntivaDao {
	...
	public void update(SiceeTDocAggiuntivaPk pk, SiceeTDocAggiuntiva dto) throws SiceeTDocAggiuntivaDaoException {
		jdbcTemplate.update("UPDATE " + getTableName() + " SET ID_DOC_AGGIUNTIVA = ?,NOME_DOC_ORIGINALE = ?,NOME_DOC,UID_INDEX = ?,ID_TIPO_DOC_AGG = ?,ID_CERTIFICATORE = ?,PROGR_CERTIFICATO = ?,ANNO = ?,DT_UPLOAD = ?,DT_DELETE = ?",dto.getIdDocAggiuntiva(),dto.getNomeDocOriginale(),dto.getNomeDoc(),dto.getUidIndex(),dto.getIdTipoDocAgg(),dto.getIdCertificatore(),dto.getProgrCertificato(),dto.getAnno(),dto.getDtUpload(),dto.getDtDelete());
	}
