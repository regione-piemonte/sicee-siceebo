...
import it.csi.sicee.siceebo.business.dao.dto.SiceeTSiapePk;
import it.csi.sicee.siceebo.business.dao.exceptions.SiceeTSiapeDaoException;
...
public class SiceeTSiapeDaoImpl extends AbstractDAO implements ParameterizedRowMapper<SiceeTSiape>, SiceeTSiapeDao
{
	...
	{
		jdbcTemplate.update("INSERT INTO " + getTableName() + " ( ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE ) VALUES ( ?, ?, ?, ? )",dto.getIdCertificatore(),dto.getProgrCertificato(),dto.getAnno(),dto.getIdTipoAzione());
		return dto.createPk();
	}
	...
	{
		jdbcTemplate.update("UPDATE " + getTableName() + " SET ID_CERTIFICATORE = ?, PROGR_CERTIFICATO = ?, ANNO = ?, ID_TIPO_AZIONE = ? WHERE ID_CERTIFICATORE = ? AND PROGR_CERTIFICATO = ? AND ANNO = ? AND ID_TIPO_AZIONE = ?",dto.getIdCertificatore(),dto.getProgrCertificato(),dto.getAnno(),dto.getIdTipoAzione(),pk.getIdCertificatore(),pk.getProgrCertificato(),pk.getAnno(),pk.getIdTipoAzione());
	}
	...
	{
		try {
			List<SiceeTSiape> list = jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE ID_CERTIFICATORE = ? AND PROGR_CERTIFICATO = ? AND ANNO = ? AND ID_TIPO_AZIONE = ?", this,idCertificatore,progrCertificato,anno,idTipoAzione);
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " ORDER BY ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE", this);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE ID_CERTIFICATORE = ? AND PROGR_CERTIFICATO = ? AND ANNO = ?", this,idCertificatore,progrCertificato,anno);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE ID_CERTIFICATORE = ? ORDER BY ID_CERTIFICATORE", this,idCertificatore);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE PROGR_CERTIFICATO = ? ORDER BY PROGR_CERTIFICATO", this,progrCertificato);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE ANNO = ? ORDER BY ANNO", this,anno);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_CERTIFICATORE, PROGR_CERTIFICATO, ANNO, ID_TIPO_AZIONE FROM " + getTableName() + " WHERE ID_TIPO_AZIONE = ? ORDER BY ID_TIPO_AZIONE", this,idTipoAzione);
		}
		catch (Exception e) {
			throw new SiceeTSiapeDaoException("Query failed", e);
		}
